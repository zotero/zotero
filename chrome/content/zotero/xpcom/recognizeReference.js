...
class ReferenceResolver {
	async resolve(reference, libraryID) {
	...
	async _getItemFromURL(url) {
		let results = await Zotero.HTTP.processDocuments(url, async function (doc) {
			try {
...
class RecognizeReference {
	...
	async resolve(reference, libraryID) {
		return this._referenceResolver.resolve(reference, libraryID);
	}
}
