				...
				if (i==77 && dbMultilingualVersion==1) {
                    ...
                    for each(row in extras) {
                        ...
                        if (extra !== row.value) {
                            ...
                            Zotero.DB.query("UPDATE items SET dateModified=DATETIME('now') WHERE itemID=?",[row.itemID]);
                        }
                    }
                    ...
                    Zotero.DB.query("UPDATE items SET clientDateModified=DATETIME('now') WHERE itemID IN (SELECT itemID FROM items NATURAL JOIN itemData NATURAL JOIN itemDataValues WHERE fieldID=22 AND value LIKE 'mlzsync1:%')");
                }
