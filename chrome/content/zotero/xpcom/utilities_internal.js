...
Zotero.Utilities.Internal = {
	...
	/**
	 * Retrieve PDF from URL using the translator framework
	 * @param {string}      url     Url used to retrieve PDF
	 * @param {Zotero.Item} parent  Zotero item to which the pdf is attached
	 * @yield {Promise}     Promise that resolves with output from itemSaver._saveAttachment.
	 */
	getPDFFromURL: Zotero.Promise.coroutine(function* (url, parent) {
		...
		return itemSaver._saveAttachment(atts[0], parent.id, () => {});
	})
}
