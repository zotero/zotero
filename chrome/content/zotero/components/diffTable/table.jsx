...
import { IconTick, IconCross, IconArrowRefresh } from '../icons';
			...
			<div className="body">
				{rows.map(row => (
					<div key={row.itemID} className="row">
						<div className="left">{
							row.status === Zotero.UpdateMetadata.ROW_SUCCEEDED && row.fields.length &&
							<button onClick={() => handleReject(row.itemID)}>↶</button>
						...
						<div className="right fields-view">
							<div className="title" onDoubleClick={() => handleDoubleClick(row.itemID)}>{row.title}</div>
