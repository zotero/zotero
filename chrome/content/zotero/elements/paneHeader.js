	...
	class PaneHeader extends XULElementBase {
		...
		init() {
			...
			this._prefsObserverIDs = [
				Zotero.Prefs.registerObserver('itemPaneHeader', () => this.render()),
				Zotero.Prefs.registerObserver('itemPaneHeader.bibEntry.style', () => this.render()),
				Zotero.Prefs.registerObserver('itemPaneHeader.bibEntry.locale', () => this.render()),
			];
		...
		notify(action, type, ids) {
			...
			if (action == 'modify' && this._item && ids.includes(this._item.id)) {
				this.render();
			}
		}
		...
		render() {
			...
			if (headerMode === 'bibEntry') {
				if (!Zotero.Styles.initialized()) {
					...
					Zotero.Styles.init().then(() => this.render());
					return;
				}
