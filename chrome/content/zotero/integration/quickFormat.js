...
var Zotero_QuickFormat = new function () {
	...
	this.onDOMContentLoaded = function(event) {
		if(event.target === document) {
			...
			referenceBox.addEventListener("click", (e) => {
				...
				// On Shift-Click, confirm the multiselection
				if (e.shiftKey && referenceBox.selectedCount > 0) {
					e.stopPropagation();
					e.preventDefault();
					Zotero_QuickFormat._bubbleizeSelected();
				}
	...
	function _buildListItem(item) {
		...
		rll.addEventListener("click", (e) => {
			let mouseMultiSelect = (Zotero.isMac && e.metaKey) || (!Zotero.isMac && e.ctrlKey);
			if (mouseMultiSelect) {
				// Focus multi-selected item on Cmd/Ctrl-mouse click
				rll.focus();
			}
			else {
				Zotero_QuickFormat._bubbleizeSelected();
