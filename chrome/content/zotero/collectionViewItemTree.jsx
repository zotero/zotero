...
class CollectionViewItemTreeRowProvider extends ItemTreeRowProvider {
	...
	async setCollectionTreeRow(collectionTreeRow) {
		...
		let idChanged = await this.itemTree.setId("item-tree-" + this.itemTree.props.id + "-" + visibilityGroup);
	...
	async notify(action, type, ids, extraData) {
		...
		// Only when we haven't already scheduled a refresh (e.g. from search mode), since refresh
		// will rebuild everything from scratch.
		if (type == 'item' && action == 'modify' && !refresh && this.itemTree.hasDependOnChildrenColumn) {
			if (!items) items = Zotero.Items.get(ids);
