...
import { parseCitations, processCitations, replaceCitations, UNMAPPED, AMBIGUOUS, MAPPED } from 'zotero/modules/rtf.mjs';
...
const Zotero_RTFScan = { // eslint-disable-line no-unused-vars, camelcase
	...
	async scanRTF() {
		...
		this.contents = (await Zotero.File.getContentsAsync(this.inputFile))
			.replace(/([^\\\r])\r?\n/, "$1 ")
			.replaceAll("\\'92", "'")
			.replaceAll("\\rquote ", "’");
		...
		for (let mapping of mappings) {
			...
			switch (mappingType) {
				...
				case AMBIGUOUS:
					...
					this.insertRows(
						items.map(item => ({ rtf: '', title: item.getField('title'), parent: row })),
	...
	async formatRTF() {
		...
		await Zotero.File.putContents(this.outputFile, content);
