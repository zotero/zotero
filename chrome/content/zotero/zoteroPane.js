		...
		for (let i = 0; i < itemIDs.length; i++) {
			...
			if (path) {
				...
				if (!externalViewer) {
					...
					var customPDFReaderPath  = Zotero.Prefs.get("extensions.zotero.fileHandler.pdf");
					if(customPDFReaderPath && item.attachmentContentType === 'application/pdf') {
						Zotero.launchFileWithGivenApplication(customPDFReaderPath, file.path);
					} else {
