...
const ZoteroStandalone = new function() {
	...
	this.onDeepTutorPaneOpen = function () {
		...
		// Check current state using both hidden and collapsed for compatibility
		var isCollapsed = deepTutorPane.hidden || deepTutorPane.getAttribute('collapsed') === 'true';
		...
		if (isCollapsed) {
			...
			// Use the proper ZoteroContextPane API instead of direct DOM manipulation
			if (typeof ZoteroContextPane !== 'undefined' && !ZoteroContextPane.collapsed) {
				Zotero.debug('Standalone: Closing context pane when opening DeepTutor pane');
			...
			if (sharedWidth) {
				...
				if (adjustedWidth > 0) {
					Zotero.debug('Standalone: Applying adjusted width to DeepTutor pane: ' + adjustedWidth + 'px (original: ' + sharedWidth + ')');
					deepTutorPane.style.width = adjustedWidth + 'px';
				} else {
					Zotero.debug('Standalone: Adjusted width too small, using default width');
				}
			}
			...
			case 'item-pane':
				...
				if (itemPane.getAttribute('collapsed') == 'true') {
					...
					if (sharedWidth) {
						...
						var adjustedWidth = parseInt(sharedWidth) + 37;
						Zotero.debug('Standalone: Applying adjusted width to item pane: ' + adjustedWidth + 'px (original: ' + sharedWidth + ')');
						itemPane.style.width = adjustedWidth + 'px';
					}
