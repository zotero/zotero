...
preferences-sync-fileSyncing-webDAVProfiles-description = Create named WebDAV file-storage profiles and assign them to individual libraries. Passwords are stored separately for each profile.
...
preferences-sync-fileSyncing-library-storage-description = Choose the file-storage backend for each library. “Default” uses { -app-name }’s existing settings above. WebDAV profiles can be assigned to only one library.
