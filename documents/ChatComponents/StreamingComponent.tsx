...
const StreamingComponent: React.FC<StreamingComponentProps> = ({
  ...
  const [streamingState, setStreamingState] = useState<StreamingStates>(
    StreamingStates.Default,
  );
  const [thinkingText, setThinkingText] = useState<string>('');
  const [responseText, setResponseText] = useState<string>('');
  const [pastStatuses, setPastStatuses] = useState<StreamingStates[]>([]);
  ...
  return (
    <div>
      ...
      {/* response
      {pastStatuses.includes(StreamingStates.Response) && (
        <StreamingStatusTag
          streamState={StreamingStates.Response}
          isCurrentTag={
            pastStatuses[pastStatuses.length - 1] === StreamingStates.Response
          }
        />
      )} */}
