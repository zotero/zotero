...
describe("Connector Server", function () {
	...
	describe("/connector/savePage", function() {
		...
		it("should translate a page if translators are available", function* () {
			var html = Zotero.File.getContentsFromURL(getTestDataUrl('article.html'));
			var promise = waitForItemEvent('add');
			var xmlhttp = yield Zotero.HTTP.request(
				'POST',
				connectorServerPath + "/connector/savePage",
				{
					...
					body: JSON.stringify({
						uri: "https://www.zotero.org/blog/indiana-university-survey-of-zotero-users/",
						html: html
					}),
					successCodes: false
				}
			...
			var item = Zotero.Items.get(ids[0]);
			assert.equal(JSON.parse(xmlhttp.responseText).items[0].title, item.getField('title'));
