describe("Zotero.QuickCopy", function () {
	...
	before(async function () {
		...
		let translatorMarkdown = Zotero.Translators.get(Zotero.Translators.TRANSLATOR_ID_NOTE_MARKDOWN);
		translatorMarkdown.cacheCode = true;
		await Zotero.Translators.getCodeForTranslator(translatorMarkdown);

		let translatorHTML = Zotero.Translators.get(Zotero.Translators.TRANSLATOR_ID_NOTE_HTML);
		translatorHTML.cacheCode = true;
		await Zotero.Translators.getCodeForTranslator(translatorHTML);
	});
	...
	describe("#reformatNoteCitations()", function () {
		...
		before(async function () {
			...
			var citationOne = { citationItems: [{ uris: [uriOne] }], properties: {} };
			var citationTwo = { citationItems: [{ uris: [uriTwo] }], properties: {} };
			...
			noteItem = createUnsavedDataObject('item', { itemType: 'note', note: noteHTML });
		...
		// getContentFromItems() calls reformatNoteCitations() internally when
		// items are notes, so we can read the reformatted output off content.text.
		...
		it("should reformat note citations in the current CSL style", function () {
			Zotero.Prefs.set('export.quickCopy.bibliographySetting', JSON.stringify({
				mode: 'bibliography', id: styleID, contentType: '', locale: ''
			}));

			let content = Zotero.QuickCopy.getContentFromItems([noteItem], getNoteFormat());
		...
		it("should use embedded item data for deleted items", async function () {
			...
			var citationOne = { citationItems: [{ uris: [uriOne] }], properties: {} };
			var citationDeleted = { citationItems: [{ uris: [uriDeleted] }], properties: {} };
			...
			// Delete the item so its URI no longer resolves -- forces the
			// fallback path through embedded data-citation-items JSON.
			...
			let content = Zotero.QuickCopy.getContentFromItems([deletedNoteItem], getNoteFormat());
			var expected = 'Before\n\n'
				+ '(firstOne lastOne and firstTwo lastTwo, Item One (n.d.).)\n\n'
	...
	describe("Smart copy", function () {
		...
		// The parent `beforeEach` re-pins prefs to the file's defaults, so
		// override the bibliography setting here for Smart copy assertions.
		beforeEach(function () {
