describe("Zotero_File_Interface", function () {
	...
	describe("#writeToClipboard()", function () {
		...
		it("should write text and html flavors directly to the clipboard", function () {
			win.Zotero_File_Interface.writeToClipboard({
				text: 'plain text',
				html: '<p>html text</p>'
			});
			assert.equal(getDataForFlavor('text/plain'), 'plain text');
			assert.equal(getDataForFlavor('text/html'), '<p>html text</p>');
		});
	});
