describe("Citation Dialog", function () {
	...
	describe("Manage entries in the citation", function () {
		...
		it("should add a locator to a just added bubble", async function () {
			...
			// Make sure item two is highlighted
			let bubbleNode = dialog.document.querySelector(`.bubble[dialogReferenceID="${CitationDataManager.items[0].dialogReferenceID}"]`);
			assert.isTrue(bubbleNode.classList.contains("just-added"));

			// Type a locator and press Enter
			let currentInput = dialog.document.getElementById("bubble-input").getCurrentInput();
			currentInput.value = "p. 10";
			...
			assert.equal(CitationDataManager.items[0].locator, "10");
		...
		it("should add a locator to a bubble before the input", async function () {
			...
			// Both items should be highlighted as just-added
			let bubbleNodeOne = dialog.document.querySelector(`.bubble[dialogReferenceID="${CitationDataManager.items[0].dialogReferenceID}"]`);
			assert.isTrue(bubbleNodeOne.classList.contains("just-added"));
			let bubbleNodeTwo = dialog.document.querySelector(`.bubble[dialogReferenceID="${CitationDataManager.items[1].dialogReferenceID}"]`);
			assert.isTrue(bubbleNodeTwo.classList.contains("just-added"));
			...
			assert.equal(CitationDataManager.items[1].locator, "10");
		...
		it("should count numeric value as a locator only after a bubble is added", async function () {
			...
			// Press Enter and make sure it is added as a locator
			currentInput.dispatchEvent(new KeyboardEvent('keydown', { key: "Enter", bubbles: true }));
	...
	describe("Helpers.extractLocator", function () {
		...
		describe("Valid locator labels", function () {
			...
			it("is a valid special numeric page locator after a new bubble is added", function () {
				let locator = dialog.Helpers.extractLocator('20-50', { bubbleJustAdded: true });
				assert.isOk(locator);
				assert.equal(locator.label, 'page');
				assert.equal(locator.locator, '20-50');
				assert.equal(locator.onlyLocator, true);
				assert.equal(locator.fullLocatorString, '20-50');
			});
		});
	});
});
