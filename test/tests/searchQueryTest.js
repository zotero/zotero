...
describe("Zotero.SearchQuery", function () {
	...
	describe("undoing a recognized field", function () {
		it("should read a field as text when its offset is marked literal", function () {
			let { tree, text } = Zotero.SearchQuery.parse("title:foo bar",
				{ literalAt: new Set([0]) });
			assert.isNull(tree);
			assert.equal(text, "title:foo bar");
		});
	});
