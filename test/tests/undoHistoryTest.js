describe("Zotero.UndoHistory", function () {
	...
	describe("custom entry state check", function () {
		afterEach(function () {
			Zotero.UndoHistory.clear();
		});
		
		it("should decline undo and discard history when the current state no longer matches the recorded new state", async function () {
			let undoSpy = sinon.spy();
			// An unrelated prior entry, to verify the decline discards all
			// history rather than just consuming the stale entry
			Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-rename-collection',
				undo: () => {},
				redo: () => {}
			});
			Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-hide-collection',
				undo: undoSpy,
				redo: () => {},
				state: { old: 'shown', new: 'hidden' },
				// An outside writer changed the state after the entry was recorded
				getState: () => 'shown'
			});
			
			assert.isFalse(await Zotero.UndoHistory.undo(), "stale undo is declined");
			assert.isFalse(undoSpy.called, "the undo callback is not invoked");
			assert.isFalse(Zotero.UndoHistory.canRedo(), "the declined entry is not moved to the redo stack");
			assert.isFalse(Zotero.UndoHistory.canUndo(), "the entire history is discarded, not just the stale entry");
		});
		
		it("should decline redo and discard history when the current state no longer matches the recorded old state", async function () {
			let current = 'hidden';
			...
			assert.isTrue(Zotero.UndoHistory.canRedo());
			assert.isTrue(Zotero.UndoHistory.canUndo(), "the unrelated prior entry is still undoable");
			
			// An outside writer changes the state while the entry sits on the redo stack
			current = 'hidden';
			
			assert.isFalse(await Zotero.UndoHistory.redo(), "stale redo is declined");
			assert.isFalse(redoSpy.called, "the redo callback is not invoked");
			assert.isFalse(Zotero.UndoHistory.canUndo(), "the entire history is discarded, not just the stale entry");
			assert.isFalse(Zotero.UndoHistory.canRedo(), "history is discarded");
		});
		
		it("should compare states with the provided comparator instead of strict equality", async function () {
			let current = { show: false };
			let isEqual = sinon.spy((a, b) => a.show === b.show);
			Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-hide-collection',
				undo: () => current = { show: true },
				redo: () => current = { show: false },
				// Distinct object instances: strict equality would always decline
				state: { old: { show: true }, new: { show: false } },
				getState: () => current,
				isEqual
			});
			
			assert.isTrue(await Zotero.UndoHistory.undo(), "undo proceeds when the comparator matches the states");
			assert.isTrue(isEqual.called, "the comparator is used");
			assert.isTrue(current.show, "the undo callback ran");
			assert.isTrue(Zotero.UndoHistory.canRedo());
		});
		
		it("should treat a replay that does not produce the recorded state as a failure", async function () {
			// An unrelated prior entry, to verify the failure discards all
			// history rather than just consuming the failed entry
			Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-rename-collection',
				undo: () => {},
				redo: () => {}
			});
			// e.g. the replay resolved through a missing UI handle and silently no-oped
			Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-hide-collection',
				undo: () => {},
				redo: () => {},
			...
			assert.isFalse(await Zotero.UndoHistory.undo(), "a no-op replay is not reported as a successful undo");
			assert.isFalse(Zotero.UndoHistory.canRedo(), "the entry is not moved to the redo stack");
			assert.isFalse(Zotero.UndoHistory.canUndo(), "the entire history is discarded, not just the failed entry");
		});
		
		it("should reject an entry that provides only one of state and getState", function () {
			assert.throws(() => Zotero.UndoHistory.pushCustomEntry({
				action: 'undo-action-hide-collection',
				undo: () => {},
				redo: () => {},
	...
	describe("showing and hiding a virtual collection", function () {
		...
		it("should create an undo action when hiding Duplicate Items", async function () {
			// Make sure Duplicate Items is visible before hiding it
			Zotero.Prefs.clear('duplicateLibraries');
			await cv.refresh();
			assert.notStrictEqual(cv.getRowIndexByID("D" + userLibraryID), false, "Duplicate Items is visible");
			
			Zotero.UndoHistory.clear();
			
			await cv.selectByID("D" + userLibraryID);
			await zp.deleteSelectedCollection();
			await cv.refresh();
			
			assert.isFalse(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"Duplicate Items is hidden"
			);
			assert.isFalse(cv.getRowIndexByID("D" + userLibraryID), "Duplicate Items row is gone");
			
			assert.isTrue(Zotero.UndoHistory.canUndo(), "hiding creates an undo entry");
			assert.ok(Zotero.UndoHistory.getUndoAction(), "an undo action is recorded");
			
			// Undo should bring Duplicate Items back. undo() awaits the custom
			// entry's callback, which refreshes and reselects, so no extra wait.
			await Zotero.UndoHistory.undo();
			assert.isTrue(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"Duplicate Items is visible again after undo"
			);
			assert.notStrictEqual(cv.getRowIndexByID("D" + userLibraryID), false, "Duplicate Items row is restored after undo");
		});
		
		it("should record showing a hidden virtual collection and support undo/redo", async function () {
			// Start with Duplicate Items hidden
			Zotero.Prefs.set('duplicateLibraries', `{"${userLibraryID}": false}`);
			await cv.refresh();
			assert.isFalse(cv.getRowIndexByID("D" + userLibraryID), "Duplicate Items starts hidden");
			
			Zotero.UndoHistory.clear();
			
			// Show it via the "Show Duplicate Items" context-menu callsite. The
			// handler fires setVirtual without awaiting, so settle the tree ourselves.
			await cv.selectByID("L" + userLibraryID);
			zp.onCollectionContextMenuSelect({ target: { id: 'showDuplicates' }, stopPropagation() {} });
			await cv.refresh();
			
			assert.isTrue(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"Duplicate Items is shown"
			);
			assert.notStrictEqual(cv.getRowIndexByID("D" + userLibraryID), false, "Duplicate Items row appears");
			
			// Showing should be undoable...
			assert.isTrue(Zotero.UndoHistory.canUndo(), "showing creates an undo entry");
			await Zotero.UndoHistory.undo();
			assert.isFalse(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"Duplicate Items is hidden again after undo"
			);
			
			// ...and redoable
			assert.isTrue(Zotero.UndoHistory.canRedo(), "showing can be redone");
			await Zotero.UndoHistory.redo();
			assert.isTrue(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"Duplicate Items is shown again after redo"
			);
			assert.notStrictEqual(cv.getRowIndexByID("D" + userLibraryID), false, "Duplicate Items row restored after redo");
		});
		
		it("should replay undo against the active pane's tree rather than the recording tree", async function () {
			// The undo/redo callbacks must resolve the live collection tree at replay
			// time. Capturing the recording tree breaks undo once the original window
			// is closed and reopened (possible on macOS).
			Zotero.Prefs.clear('duplicateLibraries');
			await cv.refresh();
			assert.notStrictEqual(cv.getRowIndexByID("D" + userLibraryID), false, "Duplicate Items is visible");
			...
			// Stand in a fresh pane/tree for the one that recorded the entry. The
			// stub applies the pref change like the real method, so the replay
			// passes the entry's state check.
			let freshTree = {
				toggleVirtualCollection: sinon.stub().callsFake((libraryID, type, show) => {
					Zotero.Prefs.setVirtualCollectionStateForLibrary(libraryID, type, show);
					return Promise.resolve();
				})
			};
			let stub = sinon.stub(Zotero, 'getActiveZoteroPane').returns({ collectionsView: freshTree });
			let undone;
			try {
				undone = await Zotero.UndoHistory.undo();
			}
			finally {
				stub.restore();
			}
			
			assert.isTrue(undone, "undo succeeds when the replay produces the recorded state");
			assert.isTrue(freshTree.toggleVirtualCollection.calledOnce, "undo replays on the active pane's tree");
			assert.deepEqual(
				freshTree.toggleVirtualCollection.firstCall.args,
				[userLibraryID, 'duplicates', true, true],
				"undo re-shows the hidden Duplicate Items on the active tree"
			);
		});
		
		it("should decline undo when the collection's visibility changed out of band", async function () {
			// Start visible, then hide with undo recording
			Zotero.Prefs.clear('duplicateLibraries');
			await cv.refresh();
			Zotero.UndoHistory.clear();
			
			// An unrelated prior entry, to verify the decline discards all history
			let collection = await createDataObject('collection', { name: 'Original' });
			collection.name = 'Modified';
			await collection.saveTx({ undoAction: 'undo-action-rename-collection' });
			...
			// An out-of-band writer re-shows the collection without recording undo
			await zp.setVirtual(userLibraryID, 'duplicates', true);
			
			assert.isFalse(await Zotero.UndoHistory.undo(), "undo of the stale entry is declined");
			assert.isFalse(Zotero.UndoHistory.canUndo(),
				"the entire history is discarded, not just the stale entry");
			assert.isTrue(
				Zotero.Prefs.getVirtualCollectionStateForLibrary(userLibraryID, 'duplicates'),
				"the out-of-band change is preserved"
			);
		});
	});
});
