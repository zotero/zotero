...
describe("Zotero.Utilities.Internal", function () {
	...
	describe("#onDragItems()", function () {
		...
		afterEach(function () {
			Zotero.Prefs.clear('export.quickCopy.preferredFormatOnDrag');
			Zotero.Prefs.clear('export.quickCopy.bibliographySetting');
			Zotero.Prefs.clear('export.noteQuickCopy.setting');
		});
		...
		['bibliography', 'export'].forEach(function (preferredFormat) {
			it(`should produce CSL-formatted plain text when dragging a note regardless of preferredFormat=${preferredFormat}`, function () {
				Zotero.Prefs.set('export.quickCopy.preferredFormatOnDrag', preferredFormat);
				Zotero.Prefs.set('export.quickCopy.bibliographySetting', JSON.stringify({ mode: 'bibliography', id: styleID, contentType: '', locale: '' }));
				Zotero.Prefs.set('export.noteQuickCopy.setting', JSON.stringify({
					mode: 'export',
				...
				Zotero.Utilities.Internal.onDragItems(event, [noteItem.id]);
		...
		it("should use embedded item data for deleted items when dragging a note", async function () {
			// Create items and build a note with embedded citation data
			...
			// Build note with data-citation-items containing embedded item data
			...
			// Delete the item so its URI no longer resolves
			...
			var event = createFakeEvent();
			Zotero.Utilities.Internal.onDragItems(event, [deletedNoteItem.id]);
